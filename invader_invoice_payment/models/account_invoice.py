...
class AccountInvoice(models.Model):
    ...
    authorized_transaction_ids = fields.Many2many(
        "payment.transaction",
        compute="_compute_validated_transaction_ids",
        string="Authorized Transactions",
        copy=False,
        readonly=True,
    )
    done_transaction_ids = fields.Many2many(
        "payment.transaction",
        compute="_compute_validated_transaction_ids",
    ...
    def _compute_validated_transaction_ids(self):
        for rec in self:
            rec.authorized_transaction_ids = rec.transaction_ids.filtered(
                lambda t: t.state == "authorized"
            )
            rec.done_transaction_ids = rec.transaction_ids.filtered(
