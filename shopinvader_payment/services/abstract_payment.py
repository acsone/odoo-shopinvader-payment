...
class AbstractPaymentService(AbstractComponent):
    ...
    def _get_payment_mode(self, provider):
        payment_modes = self.shopinvader_backend.payment_method_ids.mapped(
            "payment_mode_id"
        )
    ...
    def _validator_add_payment(self):
        validator = {
            "payment_mode": {
                "type": "dict",
                "schema": {
                    "id": {"coerce": int, "nullable": True, "required": True}
                },
            }
        }
    ...
    def _validator_check_payment(self):
        validator = {"provider_name": {"type": "string", "required": True}}
    ...
    def _set_payment_mode(self, target, params):
        ...
        if payment_mode_id not in available_payment_mode_ids:
            ...
            newvals = target.play_onchanges(vals, ["payment_mode_id"])
            vals.update(newvals)
            target.write(vals)
    ...
    def _get_return_url(self, provider_name):
        ...
        return "{}/invader/{}/check_payment/{}".format(
            self.shopinvader_backend.location, self._usage, provider_name
        )
    ...
    def _execute_payment_action(
        ...
        #  .
        # /!\ /!\ /!\ /!\ /!\ /!\ /!\ /!\ /!\ /!\ /!\ /!\ /!\ /!\ /!\ /!\ /!\
        # As this Component is abstract, it shouldn't call any super.
        # But as payment service can be specific to a payment provider,
        # we have to call a super (if defined) to have these specifics values.
        # This dirty code shouldn't be migrated in V12.0 but should be solved
        # by changing the core-design of payment modules.
        ...
        if transaction.url:
            ...
            values.update(self._action_after_payment(target))
            return values
        else:
    ...
    def _include_payment(self, target, values):
        ...
        :return: dict
        """
        values.update({"payment": self._get_payment_info(target)})
        return values
    ...
    def _convert_one_sale(self, sale):
        ...
        values.update(
            super(AbstractPaymentService, self)._convert_one_sale(sale)
        )
        return values
